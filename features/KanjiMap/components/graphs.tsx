...
import { useGraphStore } from "@/features/KanjiMap/store/graphStore";
import { GraphLegend } from "./graph-legend";
import { buildKanjiHref, type MobileTabKey } from "@/features/KanjiMap/lib/kanji-routing";
import {
  resolveKanjiId,
...
export const Graphs: React.FC<Props> = ({
  ...
  const rotate = useGraphStore((s) => s.rotate);
  const outLinks = useGraphStore((s) => s.outLinks);
  const particles = useGraphStore((s) => s.particles);
  const setStyle = useGraphStore((s) => s.setStyle);
  const setRotate = useGraphStore((s) => s.setRotate);
  const setOutLinks = useGraphStore((s) => s.setOutLinks);
  const setParticles = useGraphStore((s) => s.setParticles);
